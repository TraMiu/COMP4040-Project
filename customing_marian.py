...
class MarianEncoder(MarianPreTrainedModel):
    ...
    def __init__(self, config: MarianConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.projection = nn.Linear(self.embed_dim * 3, self.embed_dim)
    ...
    def forward(
        ...
        if is_tgt:
            ...
            augment = torch.ones_like(hidden_states)
            # print(hidden_states.shape, hidden_states)
            hidden_states = torch.cat([hidden_states, augment, hidden_states], dim=-1)
        ...
        if is_src:
            ...
            augment = torch.ones_like(hidden_states)
            # print(hidden_states.shape, hidden_states)
            hidden_states = torch.cat([hidden_states, hidden_states, augment], dim=-1)
            # print(hidden_states.shape)
            hidden_states = self.projection(hidden_states)
