...
class MarianEncoder(MarianPreTrainedModel):
    ...
    def __init__(self, config: MarianConfig, embed_tokens: Optional[nn.Embedding] = None):
        ...
        self.projection = nn.Linear(28* 3, 28)
    ...
    def forward(
        ...
        is_tgt: bool = False,  # Added parameter to indicate target input
        is_src: bool = False,  # Added parameter to indicate source input
        ...
        if inputs_embeds is None:
            ...
            if is_tgt:
                # Assume hidden_states are the output to be augmented
                augment = torch.ones_like(inputs_embeds)
                print(inputs_embeds.shape, inputs_embeds)
                inputs_embeds = torch.cat([inputs_embeds, augment, inputs_embeds], dim=1)
                # print(hidden_states.shape)
                tensor = inputs_embeds.permute(0, 2, 1).reshape(1 * 512, 3 * 28)
                tensor_projected = self.projection(tensor)

                # Reshape back to (b, d_model, len)
                inputs_embeds = tensor_projected.view(1, 512, 28).permute(0, 2, 1)
            ...
            if is_src:
                ...
                inputs_embeds = torch.cat([inputs_embeds, inputs_embeds, augment], dim=-1)
                # print(hidden_states.shape)
                inputs_embeds = self.projection(inputs_embeds)
...
class MarianMTModel(MarianPreTrainedModel):
    ...
    def forward(
        ...
        output_hidden_states: Optional[bool] = None,
        return_dict: Optional[bool] = None,
        is_tgt: bool = False, # Added parameter,
        is_src: bool = False  # Added parameter,
    ) -> Seq2SeqLMOutput:
        r"""
